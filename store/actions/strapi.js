export default {
    ...
    async getAllMarkets({commit, state}){
        ...
        let pageData
        if(state.lang === 'eng')
            pageData = await this.$axios.$get(`${process.env.BASE_URI_STRAPI}/api/single-markets/?populate[imgLogo][populate]=%2A&populate[imgMiniarure][populate]=%2A&populate[imgBanner][populate]=%2A&populate[imgCarousel][populate]=%2A`)
        else 
            pageData = await this.$axios.$get(`${process.env.BASE_URI_STRAPI}/api/single-markets/?populate[imgLogo][populate]=%2A&populate[imgMiniarure][populate]=%2A&populate[imgBanner][populate]=%2A&populate[imgCarousel][populate]=%2A`)
        
        //console.log(pageData)
        //commit('StateAssign', {markets: pageData.data})
